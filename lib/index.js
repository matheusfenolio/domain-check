...
const easy_http_code_1 = require("easy-http-code");
...
const requestHttp = async (host) => {
    ...
    try {
        if (validateURL(host.host) || host.bypassUrlValidation) {
            ...
            return { isAlive: true, code: easy_http_code_1.HttpMessage(response.status) };
        }
        else {
        ...
        return { isAlive: false, code: lodash_1.default.isNil(err.code) ? easy_http_code_1.HttpMessage(err.response.status) : easy_http_code_1.HttpMessage(err.code) };
    }
};
